    ...
    class Procesos
    {
        ...
        public static void SendFE(string docentry, string docNum, string prefijo, string codeLog, string typeDoc, Boolean reSend)
        {
            ...
            else if (Procesos.proveedor == "CX")
            {
                ...
                Conexus.ResponseType oRespoonse =  MetodosConexus.setDocumento(Doc, impDoc);
                sRequest = requestSend;
                UpdateLogConexus(codeLog, oRespoonse, sRequest);
        ...
        //Actualizacion Log despues de envio a Carvajal
        public static void UpdateLogConexus(string codeline, Conexus.ResponseType response, string srequest)
        {
            try
            {
                ...
                if (response.CodigoTransaccion == "ERR")
                {
                        tbl.UserFields.Fields.Item("U_Resultado").Value = response.Detalles;
                        tbl.UserFields.Fields.Item("U_Status").Value = "3";
                        tbl.UserFields.Fields.Item("U_Archivo_PDF").Value = "";
                        tbl.UserFields.Fields.Item("U_Enlace_XML").Value = "";
                }
                else
                {
                    ...
                    if(string.IsNullOrEmpty(response.XML))
                        tbl.UserFields.Fields.Item("U_Enlace_XML").Value = response.XML;
                }

                
                lRetCode = tbl.Update();
                Procesos.EscribirLogFileTXT("Updatelog: " + lRetCode);
                if (lRetCode != 0)
                {
                    oCompany.GetLastError(out lRetCode, out sErrMsg);
                    Procesos.EscribirLogFileTXT("updateLog: " + sErrMsg);
            ...
            try
            {
                ...
                if (oRS.RecordCount > 0)
                {
                    ...
                    for (int i = 0; i < ResultQuery.Rows.Count; i++) //Looping through rows
                    {
                        ...
                        {
                            idLog = Convert.ToString(ResultQuery.Rows[i]["Code"]); //Getting value CodeLog
                            MetodosConexus.getTransaccionbyId(idLog);
                        }
                    }
                }
                System.Runtime.InteropServices.Marshal.ReleaseComObject(oRS);
