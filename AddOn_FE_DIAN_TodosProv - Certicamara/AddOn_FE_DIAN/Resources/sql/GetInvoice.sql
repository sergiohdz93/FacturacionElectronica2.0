Select 
'FE' as "tipoDocumento", '1.0' as "versionDocumento", Case When IsNull(A9."U_Ambiente",'02') = '02' Then 'false' Else 'true' End "registrar", A9."U_idEmpresa" as "control", 
...
Left Join (Select "DocEntry",Count(*) as "NLineas", Sum("LineTotal") as "SubTotal" From "INV1" Group By "DocEntry") A6 On A0."DocEntry" = A6."DocEntry"
Left Join (Select "DocEntry", Sum("BaseSum") as "baseimpu" From "INV4" Where "ExpnsCode" = -1 Group By "DocEntry") A7 On A0."DocEntry" = A7."DocEntry"
...
Left Join INV4 A15 On A1."DocEntry" = A15."DocEntry" And A1."LineNum" = A15."LineNum" And A15."ExpnsCode" = -1
...
Left Join (
			...
			From "@FEDIAN_SN" A1
			Inner Join "@FEDIAN_SNTRI" A2 On A1.Code = A2.Code
			Where A1."Code" = (Select "CardCode" From OINV Where "DocEntry" = {0})
